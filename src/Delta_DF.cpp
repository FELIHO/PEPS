...
Delta_DF::Delta_DF(const Delta_DF &O) {

}

double Delta_DF::delta(const PnlMat *past, double t, PnlVect *delta, PnlVect *ic_delta, AssetModel *mod, Option *opt, PnlRng *rng, int nbSamples) {
	TrendtimeSteps = opt->T_ / opt->nbTimeSteps_;
	...
	double exp_ = exp(-mod->r_*(opt->T_ - t));
	// calcul delta
